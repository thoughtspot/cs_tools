...
class SecurityHome(Screen):
    ...
    tables: reactive[list[_types.APIResult]] = reactive(list, recompose=True)
    groups: reactive[list[_types.APIResult]] = reactive(list, recompose=True)
    ...
    def _make_table_selection_text(self, table_info: _types.APIResult) -> str:
        ...
        metadata_type = type_mapping.get(table_info["metadata_header"]["type"])
        return f"{metadata_type} [dim]|[/] {name}"
