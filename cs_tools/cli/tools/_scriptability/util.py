...
class GUIDMapping:
    ...
    def set_mapped_guid(self, from_guid, to_guid):
        """Sets the guid mapping from the old to the new.  You have to set both to make sure both are in the file."""
        self.guid_mapper[from_guid] = (self.from_env, from_guid)
        self.guid_mapper[from_guid] = (self.to_env, to_guid)

    def disambiguate(self, tml: TMLObject, delete_unmapped_guids: bool = False):
        """
        Replaces source GUIDs with target.
        :param tml: A TLM object to replace GUIDs for.
        :param delete_unmapped_guids: If true, unmapped GUIDs will be removed.
        """
    ...
    def save(self):
        ...
        self.guid_mapper.save(path=self.path, info={"generated-by": "scriptability"})
