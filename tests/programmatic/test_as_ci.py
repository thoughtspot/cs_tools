...
def test_build_config_from_envvar():
    ...
    assert fake.thoughtspot.password == conf.thoughtspot.password == "eNorSk3MyanULS4tSC3SLU5NLkot0S0uKcrPS9ctSCwuLs8vSgEA-k4OAw=="
    # fmt: on
...
def test_integration_cli_searchable():
    """
    For running in serverless environments, we should be able to parse
    environment variables.
    """
    ...
    result = (
        ...
        .invoke(
            "bi-server", 
            shlex.split(f"--from-date {jan} --to-date {jan} --syncer csv://directory={out} --config ENV:{env}")
        )
    )
    # fmt: on
    ...
    assert const.TEST_DATA_DIRECTORY.joinpath("ts_bi_server.csv").exists()
